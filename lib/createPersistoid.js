...
function createPersistoid(config) {
  ...
  var actionBlacklist = config.actionBlacklist || null;
  ...
  var update = function update(state, action) {
    ...
    Object.keys(state).forEach(function (key) {
      ...
      if (!passActionBlacklist(action)) return;
  ...
  function passActionBlacklist(action) {
    var pass = true;
    if(actionBlacklist) {
      for (var i=0; i<actionBlacklist.length; i++) {
        var blackWord = actionBlacklist[i];
        if(action.indexOf(blackWord) !== -1){
          pass = false;
          break;
        }
      }
