...
function persistReducer(config, baseReducer) {
  ...
  var _purge = false;
  var _paused = true;
  var conditionalUpdate = function conditionalUpdate(state, action) {
    // update the persistoid only if we are rehydrated and not paused
    state._persist.rehydrated && _persistoid && !_paused && _persistoid.update(state, action);
    return state;
  };
  ...
  return function (state, action) {
    ...
    if (action.type === _constants.PERSIST) {
      ...
      if (_purge) return _extends({}, restState, {
        ...
        return conditionalUpdate(_newState, action);
      }
    }
    ...
    if (newState === restState) return state;else {
      ...
      return conditionalUpdate(newState, action);
    }
  };
}
